...
app.post('/api/sd-transfer/:pilot/confirm', (req, res) => {
  const pilotId=req.params.pilot; const sessionId=Number(req.body?.session_id||0); const mode=String(req.body?.mode||'CALM').toUpperCase()==='DYN'?'DYN':'CALM'; const pilot=db.prepare('SELECT tag FROM pilots WHERE pilot_id=?').get(pilotId); const batchSize=pilot?.tag==='sd card'?4:4; const flights=db.prepare("SELECT * FROM flights WHERE session_id=? AND pilot_id=? AND mode=? AND status='completed' AND result='needs_sd_transfer' AND sd_transfer_ack=0 ORDER BY id").all(sessionId,pilotId,mode); if(flights.length!==batchSize)return res.status(400).json({error:`Exactly ${batchSize} completed flights are required.`});
